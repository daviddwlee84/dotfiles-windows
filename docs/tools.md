...
| herdr multiplexer | herdr, a native Windows terminal multiplexer (**preview beta**). No scoop/winget manifest — installs via herdr.dev's `irm \| iex` script; config managed at `~/.config/herdr/config.toml` (pwsh as the default shell). Also installs the [herdr-plus](https://github.com/cloudmanic/herdr-plus) plugin that backs `prefix+y` (Quick Actions) and `prefix+O` (Projects) — **needs `go`** (its Windows build is `go build`), so it is skipped with a message when go is absent (`scoop install go`, or enable *Extra runtimes*). See [rationale](rationale.md#terminal-multiplexer-wezterm-stable-default-herdr-native-beta). |
