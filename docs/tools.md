...
now ships an official Windows CLI and installs with the coding-agent bundle.
The installer selects the latest stable release, verifies its published SHA-256,
and probes the binary before placing it in `~/.local/bin`. No Go build is needed.
...
follow the same connection as PowerShell downloads; it does not change package
registries, persistent environment variables, or the public fallback policy.
...
just upgrade-dev       # verified official Windows dev-cli release (personal suite)
...
updates and config merges preserve the installed channel.

```powershell
just upgrade-personal-check # preview selected and installed packages
...
and result. Noninteractive calls require the tool's normal approval flag.
PowerShell completion is cached from the installed executable and survives
profile reload; shell startup never installs missing tools.
