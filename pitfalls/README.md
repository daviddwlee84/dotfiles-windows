...
| [`local-bin-translate-shadows-scoop-shim`](local-bin-translate-shadows-scoop-shim.md) | `scoop update translate` succeeds but `translate --version` is unchanged, two paths from `Get-Command translate -All`, `~\.local\bin` precedes `~\scoop\shims` on PATH, tool moved from `go install` to scoop keeps old behaviour | fixed |
