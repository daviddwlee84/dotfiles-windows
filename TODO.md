...
- [ ] **[?/L] specstory: capture transcripts without a resident wrapper** — `specstory run` is the agent's parent process, so every pane pays for a second resident parser runtime (measured GB-scale on macOS). `_herdr_wrap_agent` in `dot_config/powershell/profile.d/25_herdr.ps1` has the same shape. Design + spike live in the Unix repo — one backlog doc, not two: `dotfiles/backlog/specstory-detached-transcript-capture.md` (pitfall: `dotfiles/pitfalls/specstory-run-parent-grows-to-multi-gigabyte-rss.md`). Mirror here once the session-id discovery question is settled there.
...
- [ ] **[?/L] mole on Windows — re-evaluate when the `windows` branch stops warning people off it** — [tw93/Mole](https://github.com/tw93/Mole) is adopted on the macOS side (`installMole`, homebrew formula; `mo purge` reclaims `node_modules`/`target`/`.venv` across every checkout). Its `windows` branch is a native PowerShell rewrite, but upstream's own README opens with `> [!WARNING] Experimental Status: The Windows version is currently not mature… please do not use this tool`. It sits at 1.30.0 against main's 1.53.0 (three Windows prereleases ever, latest 2026-08-16, vs roughly weekly macOS releases); `packaging/{scoop,winget,chocolatey}` are all marked not-published and point at a contributor fork; and installation is `git clone --branch windows` + PATH edits with `mo update` implemented as `git pull` — the shape this repo deliberately avoids. **Re-evaluate when all three hold**: the warning is gone, a real scoop or winget manifest exists, and the Windows version has caught up with main. Compatibility evidence and the platform-agnostic decision live in the superproject at `docs/mole-macos-only.md`; the macOS implementation is in `dotfiles/` (`docs/tools/mole.md`). No backlog doc here on purpose — no code lands in this repo until that re-evaluation passes.
- [ ] **[?/M] Agent external-editor context: Windows parity** — Track the Unix-owned agent-external-editor-context investigation in daviddwlee84/dotfiles (backlog/agent-external-editor-context.md); mirror adopted prompt-boundary, Neovim and editor-wait behavior with native PowerShell/ConPTY validation. Shared decision: daviddwlee84/dotfiles-all docs/agent-external-editor-context.md. Keep one full research note in Unix.
