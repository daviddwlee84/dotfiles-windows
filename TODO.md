...
- [ ] **[M] Manage `~/.gitconfig` (currently unmanaged entirely)** — no `dot_gitconfig*` exists in the source tree, so `core.autocrlf=input`, `pull.rebase=true` and the CRLF-pitfall fix are all machine-local and unreproducible. Parent repo has `modify_dot_gitconfig.tmpl`, but a verbatim port has three landmines: it's bash (needs a `.ps1` rewrite like herdr's overlay), its baseline sets `core.hooksPath` which would silently disarm the per-clone `pre-commit` hook, and its destructive merge would drop the load-bearing `core.autocrlf=input`. → [research](backlog/manage-gitconfig-via-chezmoi.md)
